...
def main():
    ...
    external_logs_dir = os.environ.get("EXTERNAL_LOGS_DIR", "/external_logs")
    
    # Check if external logs exist
    has_external_logs = os.path.exists(external_logs_dir) and os.path.isdir(external_logs_dir)
    
    # Sidebar configuration
    st.sidebar.header("Configuration")
    
    # Select log source
    log_source_options = ["Sample Logs", "External System Logs"] if has_external_logs else ["Sample Logs"]
    log_source_type = st.sidebar.radio("Log Source", log_source_options)
    
    if log_source_type == "Sample Logs":
        log_source = default_logs_dir
    else:
        log_source = external_logs_dir
        
    # Allow manual override of log source directory
    log_source = st.sidebar.text_input("Log Source Directory", value=log_source)
    ...
    with st.sidebar.expander("Help"):
        ...
        1. Select the log source (Sample or External System Logs)
        2. Or enter a custom directory containing your log files
        3. Enter a search term to filter logs (e.g., "error")
